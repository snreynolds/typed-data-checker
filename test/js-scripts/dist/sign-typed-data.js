...
var init_transaction = __esm({
  "node_modules/viem/_esm/errors/transaction.js"() {
    ...
    TransactionExecutionError = class extends BaseError {
      constructor(cause, { account: account2, docsPath: docsPath3, chain, data: data2, gas, gasPrice, maxFeePerGas, maxPriorityFeePerGas, nonce, to, value }) {
...
function encodeArray(value, { length, param }) {
  ...
  if (dynamic || dynamicChild) {
    ...
    if (dynamic) {
      ...
      return {
        dynamic: true,
        encoded: preparedParams.length > 0 ? concat([length2, data2]) : length2
      };
    }
    if (dynamicChild)
...
function encodeFunctionData(parameters) {
  ...
  const data2 = "inputs" in abiItem && abiItem.inputs ? encodeAbiParameters(abiItem.inputs, args2 ?? []) : void 0;
  return concatHex([signature, data2 ?? "0x"]);
}
...
function decodeAbiParameters(params, data2) {
  const bytes = typeof data2 === "string" ? hexToBytes2(data2) : data2;
  const cursor = createCursor(bytes);
  if (size(bytes) === 0 && params.length > 0)
    throw new AbiDecodingZeroDataError();
  if (size(data2) && size(data2) < 32)
    throw new AbiDecodingDataSizeTooSmallError({
      data: typeof data2 === "string" ? data2 : bytesToHex2(data2),
      params,
      size: size(data2)
    });
  let consumed = 0;
  ...
  for (let i = 0; i < params.length; ++i) {
    ...
    const [data3, consumed_] = decodeParameter(cursor, param, {
      staticPosition: 0
    });
    consumed += consumed_;
    values.push(data3);
...
function decodeArray(cursor, param, { length, staticPosition }) {
  ...
  if (hasDynamicChild(param)) {
    ...
    for (let i = 0; i < length; ++i) {
      ...
      const [data2] = decodeParameter(cursor, param, {
...
function decodeBytes(cursor, param, { staticPosition }) {
  ...
  if (!size3) {
    ...
    const data2 = cursor.readBytes(length);
    cursor.setPosition(staticPosition + 32);
    return [bytesToHex2(data2), 32];
  }
...
function decodeTuple(cursor, param, { staticPosition }) {
  ...
  if (hasDynamicChild(param)) {
    ...
    for (let i = 0; i < param.components.length; ++i) {
      ...
      const [data2, consumed_] = decodeParameter(cursor, component, {
        staticPosition: start
      });
      consumed += consumed_;
      value[hasUnnamedChild ? i : component?.name] = data2;
    }
    cursor.setPosition(staticPosition + 32);
    return [value, 32];
  }
...
function decodeString(cursor, { staticPosition }) {
  ...
  const data2 = cursor.readBytes(length, 32);
...
function decodeErrorResult(parameters) {
  ...
  const signature = slice(data2, 0, 4);
  if (signature === "0x")
    throw new AbiDecodingZeroDataError();
...
function encodeDeployData(parameters) {
  ...
  if (!description.inputs || description.inputs.length === 0)
    throw new AbiConstructorParamsNotFoundError({ docsPath: docsPath2 });
  const data2 = encodeAbiParameters(description.inputs, args2);
  ...
  return {
    ...
    async schedule(args2) {
      ...
      if (hasActiveScheduler) {
        setScheduler({ args: args2, resolve: resolve2, reject });
        return promise;
      }
      setScheduler({ args: args2, resolve: resolve2, reject });
...
module.exports = __toCommonJS(sign_typed_data_exports);
...
function toBlobs(parameters) {
  ...
  while (active) {
    ...
    while (size3 < fieldElementsPerBlob) {
      const bytes = data2.slice(position, position + (bytesPerFieldElement - 1));
      blob.pushByte(0);
...
function serializeTransactionEIP7702(transaction, signature) {
  const { authorizationList, chainId, gas, nonce, to, value, maxFeePerGas, maxPriorityFeePerGas, accessList, data: data2 } = transaction;
...
function serializeTransactionEIP4844(transaction, signature) {
  const { chainId, gas, nonce, to, value, maxFeePerBlobGas, maxFeePerGas, maxPriorityFeePerGas, accessList, data: data2 } = transaction;
...
function serializeTransactionEIP1559(transaction, signature) {
  const { chainId, gas, nonce, to, value, maxFeePerGas, maxPriorityFeePerGas, accessList, data: data2 } = transaction;
  assertTransactionEIP1559(transaction);
  const serializedAccessList = serializeAccessList(accessList);
  const serializedTransaction = [
    toHex(chainId),
...
function serializeTransactionEIP2930(transaction, signature) {
  const { chainId, gas, data: data2, nonce, to, value, accessList, gasPrice } = transaction;
...
function serializeTransactionLegacy(transaction, signature) {
  const { chainId = 0, gas, data: data2, nonce, to, value, gasPrice } = transaction;
...
var InvalidPrimaryTypeError = class extends BaseError {
  constructor({ primaryType: primaryType2, types }) {
    super(`Invalid primary type \`${primaryType2}\` must be one of \`${JSON.stringify(Object.keys(types))}\`.`, {
...
function serializeTypedData(parameters) {
  const { domain: domain_, message: message_, primaryType: primaryType2, types } = parameters;
  const normalizeData = (struct, data_) => {
    const data2 = { ...data_ };
    for (const param of struct) {
      const { name, type } = param;
      if (type === "address")
        data2[name] = data2[name].toLowerCase();
    }
    return data2;
  };
  const domain2 = (() => {
  ...
  const message = (() => {
    if (primaryType2 === "EIP712Domain")
      return void 0;
    return normalizeData(types[primaryType2], message_);
  })();
  return stringify({ domain: domain2, message, primaryType: primaryType2, types });
}
function validateTypedData(parameters) {
  const { domain: domain2, message, primaryType: primaryType2, types } = parameters;
  ...
  if (primaryType2 !== "EIP712Domain") {
    if (types[primaryType2])
      validateData(types[primaryType2], message);
    else
      throw new InvalidPrimaryTypeError({ primaryType: primaryType2, types });
  }
}
...
function hashTypedData(parameters) {
  const { domain: domain2 = {}, message, primaryType: primaryType2 } = parameters;
  const types = {
...
function hashStruct({ data: data2, primaryType: primaryType2, types }) {
...
function encodeData({ data: data2, primaryType: primaryType2, types }) {
  const encodedTypes = [{ type: "bytes32" }];
  const encodedValues = [hashType({ primaryType: primaryType2, types })];
  for (const field of types[primaryType2]) {
...
function hashType({ primaryType: primaryType2, types }) {
  const encodedHashType = toHex(encodeType({ primaryType: primaryType2, types }));
  return keccak256(encodedHashType);
}
function encodeType({ primaryType: primaryType2, types }) {
  let result = "";
  const unsortedDeps = findTypeDependencies({ primaryType: primaryType2, types });
  unsortedDeps.delete(primaryType2);
  const deps = [primaryType2, ...Array.from(unsortedDeps).sort()];
...
function findTypeDependencies({ primaryType: primaryType_, types }, results = /* @__PURE__ */ new Set()) {
  ...
  const primaryType2 = match?.[0];
  if (results.has(primaryType2) || types[primaryType2] === void 0) {
    return results;
  }
  results.add(primaryType2);
  for (const field of types[primaryType2]) {
...
async function estimateGas(client, args2) {
  ...
  try {
    ...
    const { accessList, authorizationList, blobs, blobVersionedHashes, blockNumber, blockTag, data: data2, gas, gasPrice, maxFeePerBlobGas, maxFeePerGas, maxPriorityFeePerGas, nonce, value, stateOverride, ...rest } = await prepareTransactionRequest(client, {
      ...args2,
      parameters: (
...
async function sendTransaction(client, parameters) {
  const { account: account_ = client.account, chain = client.chain, accessList, authorizationList, blobs, data: data2, gas, gasPrice, maxFeePerBlobGas, maxFeePerGas, maxPriorityFeePerGas, nonce, value, ...rest } = parameters;
...
async function signTypedData2(client, parameters) {
  const { account: account_ = client.account, domain: domain2, message, primaryType: primaryType2 } = parameters;
  if (!account_)
    throw new AccountNotFoundError({
  ...
  validateTypedData({ domain: domain2, message, primaryType: primaryType2, types });
  if (account2.signTypedData)
    return account2.signTypedData({ domain: domain2, message, primaryType: primaryType2, types });
  const typedData = serializeTypedData({ domain: domain2, message, primaryType: primaryType2, types });
  return client.request({
...
// ../json/types.json
var types_default = { SignedPersonalData: [{ name: "me", type: "Person" }, { name: "friends", type: "Person[]" }], Person: [{ name: "name", type: "string" }, { name: "id", type: "uint256" }] };

// ../json/domain.json
var domain_default = { name: "TestSignerAccount", version: "1", chainId: 31337, verifyingContract: "0x5615dEB798BB3E4dFa0139dFa1b3D433Cc23b72f" };

// src/sign-typed-data.ts
var domain = domain_default;
...
if (args[0].endsWith(".json")) {
  const filePath = (0, import_path.resolve)(args[0]);
  jsonInput = JSON.parse((0, import_fs.readFileSync)(filePath, "utf8"));
} else {
  jsonInput = JSON.parse(args[0]);
}
var { privateKey, primaryType, data } = jsonInput;
var validPrimaryTypes = Object.keys(types_default);
if (!validPrimaryTypes.includes(primaryType)) {
...
function convertFieldType(fieldType, typesDefinition) {
  ...
  if (fixedArrayMatch) {
    const [, baseType, size3] = fixedArrayMatch;
    if (baseType in typesDefinition) {
      const components = convertStructFields(baseType, typesDefinition);
      return {
        type: `tuple[${size3}]`,
...
function convertStructFields(structName, typesDefinition) {
  const structDef = typesDefinition[structName];
  if (!structDef) {
...
function convertStructToAbi(structName, typesDefinition) {
  return [
    {
      name: "data",
      type: "tuple",
...
async function signTypedData3() {
  try {
    const abiParams = convertStructToAbi(primaryType, types_default);
